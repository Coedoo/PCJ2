...
textures: dm.ResourcePool(Texture_d3d, dm.TexHandle)
batches: dm.ResourcePool(RectBatch_D3D, dm.BatchHandle)
shaders: dm.ResourcePool(Shader_d3d, dm.ShaderHandle)
...
CompileShaderSource :: proc(renderCtx: ^dm.RenderContext, source: string) -> dm.ShaderHandle {
    ...
    if dm.IsHandleValid(shaders, handle) == false {
        // @TODO: logger
        return {}
    }
...
DrawPrimitiveBatch :: proc(ctx: ^RenderContext_d3d) {
    ...
    ctx.deviceContext->Map(ctx.gpuVertBuffer, 0, .WRITE_DISCARD, nil, &mapped)
